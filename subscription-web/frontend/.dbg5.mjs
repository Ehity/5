...
export function transactionsFromLines(lines) {
  ...
  const minusLines = lines.filter((l) => !PDF_SKIP_RE.test(l) && moneyMatches(l, false).some((m) => m.sign === "-"));
  minusLines.forEach((l) => console.error("MINUS:", JSON.stringify(l.slice(0, 90))));
  console.error("SKIP TEST Расходы:", PDF_SKIP_RE.test("• Расходы - 45 384.88 ₽"));
  ...
  for (const line of lines) {
    ...
    if (mDate && hasMoney) {
      ...
      const keep = sawMinus ? debit : hasPlus ? chosen.sign !== "+" : true;
        console.error("BR:", JSON.stringify({chosen: chosen.amount, sign: chosen.sign, isSkip, hasPlus, sawMinus, strict}));
      if (desc && !PDF_TIME_ONLY_RE.test(desc)) {
        if (chosen.amount !== null && chosen.amount !== 0 && keep) {
          console.error("PUSH:", chosen.amount, JSON.stringify(desc.slice(0,50)));txs.push({ date: d, amount: chosen.amount, description: desc.slice(0, 120) });
          lastWasSimple = true;
        } else {
    ...
    if (mDate) {
      ...
      if (pending !== null && pending.amount !== null && !pending.desc.length) { console.error("CONT-B amount:", pending.amount);
